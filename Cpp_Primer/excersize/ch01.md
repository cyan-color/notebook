...
解：
```
`for` 和 `while` 之间的主要区别在于：
当迭代次数已知时，我们通常使用 for 语句，若事先不知道迭代次数时使用 while 语句。

`while` 与 `do ... while` 之间的主要区别在于：
`do ... while`在开始时执行一次指令，之后，它的行为就像简单的`while`一样。
```
